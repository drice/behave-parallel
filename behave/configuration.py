...
class LogLevel(object):
    ...
    def parse_type(cls, levelname):
        ...
        if level is Unknown:
            message = "%s is unknown, use: %s" % (levelname, ", ".join(cls.names[1:]))
...
options = [
    ...
    (
        ("-d", "--dry-run"),
        dict(
            action="store_true", help="Invokes formatters without executing the steps."
        ),
    ),
    (
        ("-D", "--define"),
        dict(
            dest="userdata_defines",
            type=parse_user_define,
            action="append",
            metavar="NAME=VALUE",
    ...
    (
        (),  # -- CONFIGFILE only
        dict(
            dest="default_format", help="Specify default formatter (default: pretty)."
        ),
    ),
    (
        ("-f", "--format"),
    ...
    (
        (),  # -- CONFIGFILE only
        dict(
            dest="scenario_outline_annotation_schema",
    ...
    (
        ("--stage",),
        dict(
            help="""Defines the current test stage.
...
def read_configuration(path):
    ...
    for fixed, keywords in options:
        ...
        if action == "store":
            ...
            result[dest] = [s.strip() for s in config.get("behave", dest).splitlines()]
        else:
...
def setup_parser():
    ...
    parser.add_argument(
        "paths", nargs="*", help="Feature directory, file or file location (FILE:LINE)."
    )
    return parser
    ...
    def collect_unknown_formats(self):
        ...
        if self.format:
            for format_name in self.format:
                if format_name == "help" or _format_registry.is_formatter_valid(
                    format_name
                ):
                    continue
